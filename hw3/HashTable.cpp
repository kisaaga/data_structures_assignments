...
{
	List<HashedObj>& whichList = theLists[hhash(x, theLists.size())];
...
{
	theLists[hhash(x, theLists.size())].remove(x);
}
...
const HashedObj& HashTable<HashedObj>::find(const HashedObj& x) const
{
	...
	itr = theLists[hhash(x, theLists.size())].find(x);
	if (itr.isPastEnd())
...
int hhash(const Product& abc, int tableSize)
{
	string key = abc.name;
	int hashVal = 0;
...
template class HashTable<Product>;